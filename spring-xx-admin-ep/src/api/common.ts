...
export const reloadButtons = async () =>
  request<void>("/sys/reload_buttons", { method: "POST" });

export const reloadAuthorities = async () =>
  request<void>("/sys/reload_authorities", { method: "POST" });

export const uploadStorage = async (
  file: File,
  progressListener?: UploadProgressListener
) => uploadFile<UploadedFile>("/storage/upload", file, progressListener);

export const removeStorage = async (key: string) =>
  postForm<boolean>("/storage/remove", { key });
